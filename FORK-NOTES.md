...
| Audit v0.2 | V0–V3：插件 + 安装文档 `docs/obsidian-audit-install.md`；下一步 V4 U17/U18 人机验收 |
...
插件在 **知识库 vault**，不在 `CODEX_HOME`：

1. 拷贝 `obsidian-plugin/llm-wiki-audit/{main.js,manifest.json,styles.css}`  
   → `<wiki>/.obsidian/plugins/llm-wiki-audit/`  
2. Obsidian 启用 **llm-wiki Audit**  
3. 选中 `wiki/**/*.md` 原文 →「记 llm-wiki 批注」→ Codex：**处理批注**

完整步骤与 PowerShell 示例：[docs/obsidian-audit-install.md](docs/obsidian-audit-install.md)。
